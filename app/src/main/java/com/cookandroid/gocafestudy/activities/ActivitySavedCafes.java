...
public class ActivitySavedCafes extends AppCompatActivity {
    ...
    private void showCafeDetailBottomSheet(int cafeId) {
        ...
        if (images.size() > 0) Glide.with(this).load(images.get(0)).into(ivMain);
        if (images.size() > 1) Glide.with(this).load(images.get(1)).into(ivSub1);
        if (images.size() > 2) Glide.with(this).load(images.get(2)).into(ivSub2);
        if (images.size() > 3) Glide.with(this).load(images.get(3)).into(ivSub3);
        if (images.size() > 4) Glide.with(this).load(images.get(4)).into(ivSub4);
