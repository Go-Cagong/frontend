...
public class ActivityMyReviews extends AppCompatActivity {
    ...
    private static final String TAG = "AcitivityMyreviews";
        ...
        cafeApi.getCafeDetail(cafeId).enqueue(new Callback<CafeDetail>() {
            ...
            public void onResponse(Call<CafeDetail> call, Response<CafeDetail> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Toast.makeText(ActivityMyReviews.this, "카페 상세 정보 로드 실패: " + response.code(), Toast.LENGTH_SHORT).show();
            ...
            public void onFailure(Call<CafeDetail> call, Throwable t) {
                Toast.makeText(ActivityMyReviews.this, "카페 상세 정보 네트워크 오류", Toast.LENGTH_LONG).show();
    ...
    private void displayCafeDetailSheet(int cafeId, CafeDetail cafe) {
        ...
        if (images != null) {
            if (images.size() > 0)
                Glide.with(ActivityMyReviews.this).load(images.get(0)).placeholder(R.drawable.ic_camera_gray).into(ivMain);
            if (images.size() > 1)
                Glide.with(ActivityMyReviews.this).load(images.get(1)).placeholder(R.drawable.ic_camera_gray).into(ivSub1);
            if (images.size() > 2)
                Glide.with(ActivityMyReviews.this).load(images.get(2)).placeholder(R.drawable.ic_camera_gray).into(ivSub2);
            if (images.size() > 3)
                Glide.with(ActivityMyReviews.this).load(images.get(3)).placeholder(R.drawable.ic_camera_gray).into(ivSub3);
            if (images.size() > 4)
                Glide.with(ActivityMyReviews.this).load(images.get(4)).placeholder(R.drawable.ic_camera_gray).into(ivSub4);
        }
        ...
        btnReview.setOnClickListener(click -> {
            Intent intent = new Intent(ActivityMyReviews.this, ActivityReviewList.class);
        ...
        loadBookmarkState(ActivityMyReviews.this, cafeId, btnSave);

        btnSave.setOnClickListener(view -> {
            String currentText = btnSave.getText().toString();

            if (currentText.equals("저장하기")) {
                createBookmark(ActivityMyReviews.this, cafeId, btnSave);
            } else {
                deleteBookmark(ActivityMyReviews.this, cafeId, btnSave);
            }
        });
